...
CompileCachedJavaProject(){
	...
	case "$projectName" in
	'')
		echo "$MDSC_CMD: ⛔ ERROR: 'projectName' argument is required!" >&2
		set +e ; return 1
	;;
	--project-from-env)
		projectName="$MDSC_PRJ_NAME" ; [ -n "$projectName" ] || {
			echo "$MDSC_CMD: ⛔ ERROR: --project-from-env: MDSC_PRJ_NAME is not set!" >&2
			set +e ; return 1
		}
	;;
	'.'|--project-from-pwd)
		projectName="$( Distro ListDistroProjects --project '.' )" ; [ -n "$projectName" ] || {
			echo "$MDSC_CMD: ⛔ ERROR: --project-from-pwd: can't map working directory to project: $(pwd)" >&2
			set +e ; return 1
		}
	;;
	esac
	[ -f "$MDSC_SOURCE/$projectName/project.inf" ] || {
		echo "$MDSC_CMD: ⛔ ERROR: project is invalid or unknown: $projectName" >&2
		set +e ; return 1
	}
	shift
